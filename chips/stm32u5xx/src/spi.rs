...
impl<'a> Spi<'a> {
    ...
    pub fn handle_interrupt(&self) {
        ...
        let transfer_error =
            status.is_set(SR::OVR) || status.is_set(SR::MODF) || status.is_set(SR::UDR);

        if self.transfers_in_progress.get() == 0 && (status.is_set(SR::EOT) || transfer_error) {
            regs.ier.modify(
                IER::EOTIE::CLEAR + IER::OVRIE::CLEAR + IER::MODFIE::CLEAR + IER::UDRIE::CLEAR,
            );
    ...
    fn finish_transfer(&self) {
        ...
        let status = regs.sr.extract();
        let mut transfer_result = Ok(length);

        // Checks for Overrun, Underrun, and Mode fault
        if status.is_set(SR::OVR) || status.is_set(SR::MODF) || status.is_set(SR::UDR) {
            transfer_result = Err(kernel::ErrorCode::FAIL);
        }
        ...
        self.client.map(|client| {
            tx_buffer.map(|t| {
                client.read_write_done(t, rx_buffer, transfer_result);
            });
        });
    }
...
impl<'a> spi::SpiMaster<'a> for Spi<'a> {
    ...
    > {
        ...
        let Some(tx_ch) = self.dma_channel_tx.get() else {
            return Err((kernel::ErrorCode::OFF, write_buffer, read_buffer));
        };

        let Some(rx_ch) = self.dma_channel_rx.get() else {
            return Err((kernel::ErrorCode::OFF, write_buffer, read_buffer));
        };
        ...
        if count == 0 {
            return Err((kernel::ErrorCode::INVAL, write_buffer, read_buffer));
        }
        ...
        read_buffer.map(|mut rx_buffer| {
            ...
            dma.setup(rx_ch, crate::dma::DmaPeripheral::Spi1Rx, ptr, len);
            self.enable_rx();
        });
        ...
        dma.setup(tx_ch, crate::dma::DmaPeripheral::Spi1Tx, ptr, len);
        self.enable_tx();
...
impl crate::dma::DmaClient for Spi<'_> {
    ...
    fn transfer_done(&self, channel: ChannelId) {
        ...
        if self.transfers_in_progress.get() == 0 {
            // DMA may finish before SPI shifts the final byte. Let the SPI
            // interrupt complete the transfer once EOT is asserted or an SPI
            // error is detected.
            regs.ier
                .modify(IER::EOTIE::SET + IER::OVRIE::SET + IER::MODFIE::SET + IER::UDRIE::SET);
        }
    }
}
