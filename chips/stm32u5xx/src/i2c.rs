...
impl<'a> I2c<'a> {
    ...
    pub fn start_dma(
        ...
        let Some(dma) = self.dma.get() else {
            return Err((i2c::Error::NotSupported, buf));
        };
    ...
    pub fn stop_dma(&self) {
        ...
        let Some(dma_buf) = self.dma_buf.take() else {
            kernel::debug!(
                "Error in logic: stop_dma is supposed to be called only when there is a known self.dma_buf"
            );
            return;
        };
...
impl<'a> I2CMaster<'a> for I2c<'a> {
    ...
    fn write_read(
        ...
        if self.status.get() == I2cStatus::Idle {
            ...
            if let Err(e) = self.start_transfer(addr, data, I2cDirection::To) {
                self.status.set(I2cStatus::Idle);
                return Err(e);
            }
    ...
    fn read(
        ...
        if self.status.get() == I2cStatus::Idle {
            ...
            if let Err(e) = self.start_transfer(addr, data, I2cDirection::From) {
                self.status.set(I2cStatus::Idle);
                return Err(e);
            }
