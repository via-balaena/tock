...
pub struct Spi<'a> {
    ...
    nss: OptionalCell<&'a crate::pwr::Pwr>,
}
...
impl<'a> Spi<'a> {
    fn new(base_addr: StaticRef<SpiRegisters>, clock: SpiClock<'a>) -> Self {
        Self {
            ...
            nss: OptionalCell::empty(),
        }
    }

    pub fn new_spi1(rcc: &'a dyn Stm32wle5xxClocks) -> Self {
        Self::new(
            SPI1_BASE,
            SpiClock(phclk::PeripheralClock::new(
                phclk::PeripheralClockType::APB2(phclk::PCLK2::SPI1),
                rcc,
            )),
        )
    }

    pub fn new_subghzspi(rcc: &'a dyn Stm32wle5xxClocks) -> Self {
        Self::new(
            SUBGHZSPI,
            SpiClock(phclk::PeripheralClock::new(
                phclk::PeripheralClockType::APB3(phclk::PCLK3::SUBGHZSPI),
                rcc,
            )),
        )
    }
    ...
    pub fn set_nss(&self, nss: &'a crate::pwr::Pwr) {
        self.nss.replace(nss);
    }
...
struct SpiClock<'a>(phclk::PeripheralClock<'a>);
