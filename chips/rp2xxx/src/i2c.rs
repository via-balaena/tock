...
//! I2C controller for the Synopsys DW_apb_i2c, as fitted to the RP2040 and
//! the RP2350.
//!
//! Both chips carry the same controller at the same register offsets -- every
//! offset this driver declares appears at the same place in Table 1055 of the
//! RP2350 datasheet and Table 464 of the RP2040's -- so the driver lives here
...
impl<'a, C: SystemClock, R: ResetLine> I2c<'a, '_, C, R> {
    ...
    /// `name` is what an unexpected-interrupt message should call it.
    pub fn new(
        name: &'static str,
        registers: StaticRef<I2cRegisters>,
        clocks: &'a C,
    ...
    fn set_baudrate(&self, baudrate: u32) -> u32 {
        ...
        let freq_in = self.clocks.system_frequency();
    ...
    pub fn handle_interrupt(&self) {
        match self.state.get() {
            State::Uninitialized => {
                debug!("Unexpected IRQ for uninitialized {}", self.name)
            }
            State::Idle => debug!("Unexpected IRQ for idle {}", self.name),
