...
register_structs! {
    ...
    pub PioIrqRegisters {
        ...
        (0x004 => pub irq0_inte: ReadWrite<u32, IRQ0_INTE::Register>),
        // Interrupt Force for irq0
        (0x008 => pub irq0_intf: ReadWrite<u32, IRQ0_INTF::Register>),
        // Interrupt status after masking & forcing for irq0
        (0x00C => pub irq0_ints: ReadWrite<u32, IRQ0_INTS::Register>),
        // Interrupt Enable for irq1
        (0x010 => pub irq1_inte: ReadWrite<u32, IRQ1_INTE::Register>),
        // Interrupt Force for irq1
        (0x014 => pub irq1_intf: ReadWrite<u32, IRQ1_INTF::Register>),
        // Interrupt status after masking & forcing for irq1
        (0x018 => pub irq1_ints: ReadWrite<u32, IRQ1_INTS::Register>),
        (0x01C => @END),
    }
}
        ...
        match self.tx_state.get() {
            StateMachineState::Ready => {
                if self.tx_full() {
                    ...
                    self.irq_registers.irq0_inte.modify(field);
...
pub struct Pio {
    registers: StaticRef<PioRegisters>,
    irq_registers: StaticRef<PioIrqRegisters>,
    /// Which PIO block this is. How many a chip has is a fact about the chip,
    /// so this is a plain index and the chip crate names the blocks.
    index: usize,
...
impl Pio {
    ...
    ///
    /// `registers` is the block's base. `irq_registers` is where its
    /// interrupt registers start, which is not the same offset on every chip.
    /// The remaining three are the block's atomic XOR, SET and CLEAR aliases.
    pub fn new(
        index: usize,
        registers: StaticRef<PioRegisters>,
        irq_registers: StaticRef<PioIrqRegisters>,
    ...
    /// Which PIO block this is, counting from zero.
    ...
    pub fn handle_interrupt(&self) {
        let ints = &self.irq_registers.irq0_ints;
