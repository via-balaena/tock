...
//!
//! The CYW43439 radio speaks a half duplex SPI where one wire carries data in
//! both directions, which no hardware SPI block on either RP2 chip can do, so
//! it is bit banged by a PIO state machine with DMA feeding the FIFOs.
//!
//! Shared by the RP2040 and the RP2350. What differs between them is where
//! the DMA control bits sit, how many DMA channels there are, and the type of
//! a GPIO pin, so those are the type parameters.

use crate::dma::{self, DmaChannel, DmaChannelClient, DmaLayout};
...
pub struct PioGSpi<'a, L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad> {
    pio: &'a Pio,
    dma: DmaChannel<'a, L, CHANNELS>,
    clock_pin: u32,
    dio_pin: u32,
    cs_pin: &'a P,
    sm_number: SMNumber,
...
impl<'a, L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad>
    PioGSpi<'a, L, CHANNELS, P>
{
...
impl<'a, L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad> DmaChannelClient
    for PioGSpi<'_, L, CHANNELS, P>
{
...
impl<L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad> PioGSpi<'_, L, CHANNELS, P> {
...
impl<'a, L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad> SpiMasterDevice<'a>
    for PioGSpi<'a, L, CHANNELS, P>
{
...
impl<'a, L: DmaLayout, const CHANNELS: usize, P: gpio::Output + PioPad> PioSmClient
    for PioGSpi<'_, L, CHANNELS, P>
{
    fn on_irq(&self) {
        // Clear interrupt
...
/// `pio.rs`.
fn cyw43_spi_program_init<P: PioPad>(
    pio: &Pio,
    sm_number: SMNumber,
    clock_pin: u32,
    dio_pin: u32,
    ...
    sm.config(config);
    let clock_pin_handle = P::from_pin_number(clock_pin);
    let dio_pin_handle = P::from_pin_number(dio_pin);
    clock_pin_handle.select_pio(pio.index());
    dio_pin_handle.select_pio(pio.index());

    dio_pin_handle.set_pull_none();
    dio_pin_handle.set_schmitt(true);
    pio.set_input_sync_bypass(dio_pin, true);
    dio_pin_handle.set_drive_strength(DriveStrength::Drive12mA);
    dio_pin_handle.set_slew_rate(SlewRate::Fast);
    dio_pin_handle.activate_pads();

    clock_pin_handle.set_drive_strength(DriveStrength::Drive12mA);
    clock_pin_handle.set_slew_rate(SlewRate::Fast);
    clock_pin_handle.set_pull_none();
    clock_pin_handle.set_schmitt(true);
    clock_pin_handle.set_slew_rate(SlewRate::Slow);
