...
/// What this transport needs of a pin, gathered so the impl headers stay
/// readable.
///
/// A pad it can tune and point at a PIO block, on the same chip the DMA
/// channel belongs to, that it can also drive as an output.
pub trait PinFor<D: DmaChannel>:
    PioPin + PioPad<Block = D::Block> + gpio::Configure + gpio::Output
{
}

impl<D: DmaChannel, T> PinFor<D> for T where
    T: PioPin + PioPad<Block = D::Block> + gpio::Configure + gpio::Output
{
}
...
impl<'a, D: DmaChannel, P: PinFor<D>> PioGSpi<'a, D, P> {
    ...
    pub fn init(&self) {
        ...
        let config = StateMachineConfiguration {
            out_pins_count: 1,
            out_pins_base: self.dio_pin.pin_number(),
            set_pins_count: 1,
            set_pins_base: self.dio_pin.pin_number(),
            in_pins_base: self.dio_pin.pin_number(),
            side_set_base: self.clock_pin.pin_number(),
...
impl<D: DmaChannel, P: PinFor<D>> PioGSpi<'_, D, P> {
    fn dma_pull(&self, addr: u32, len: u32) {
        ...
        self.dma
            .read_word_from_pio(self.pio.number(), self.sm_number);
    }
    ...
    fn dma_push(&self, addr: u32, len: u32) {
        ...
        self.dma
            .write_word_to_pio(self.pio.number(), self.sm_number);
    }
}
...
fn cyw43_spi_program_init<D: DmaChannel, P: PinFor<D>>(
    ...
    sm.config(config);
    clock_pin.select_pio(pio.number());
    dio_pin.select_pio(pio.number());

    dio_pin.set_floating_state(gpio::FloatingState::PullNone);
    dio_pin.set_schmitt(true);
    pio.set_input_sync_bypass(dio_pin, true);
    dio_pin.set_drive_strength(DriveStrength::Drive12mA);
    dio_pin.set_slew_rate(SlewRate::Fast);
    dio_pin.activate_pads();

    clock_pin.set_drive_strength(DriveStrength::Drive12mA);
    clock_pin.set_slew_rate(SlewRate::Fast);
    clock_pin.set_floating_state(gpio::FloatingState::PullNone);
    clock_pin.set_schmitt(true);
    clock_pin.set_slew_rate(SlewRate::Slow);
    clock_pin.activate_pads();

    sm.set_pins_dirs(dio_pin.pin_number(), 1, true);
    sm.set_pins_dirs(clock_pin.pin_number(), 1, true);
