...
pub use rp2xxx::pio::{
    InterruptSources, LoadedProgram, PioBlock, PioFifoJoin, PioInterrupt, PioIrqClient,
    PioMovStatusType, PioPin, PioRxClient, PioTxClient, ProgramError, RelocatedProgram, SMNumber,
...
///
/// **`Rp2350DefaultPeripherals` does not hold this block, and `chip.rs` routes
/// only PIO0's two lines.** Arming an interrupt source on a block the chip
/// does not dispatch reaches `service_interrupt`'s `_ => false`, which is a
/// kernel panic -- so a board that builds this one has to add the dispatch
/// arms with it. The panic predates the interrupt line being armed here:
/// `next_pending_with_mask` reads `ISPR`, so the poll path finds a pending
/// interrupt whether or not its line is enabled.
pub fn new_pio1() -> Pio {
    new_pio(
        PIONumber::PIO1,
        PIO_1_BASE_ADDRESS,
...
pub fn new_pio2() -> Pio {
    new_pio(
        PIONumber::PIO2,
        PIO_2_BASE_ADDRESS,
        [interrupts::PIO2_IRQ_0, interrupts::PIO2_IRQ_1],
    )
}
