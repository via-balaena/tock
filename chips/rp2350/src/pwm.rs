...
//! Pulse wave modulation (PWM) driver for RP2350.
//!
...
//! # Examples
//!
//! The block has twelve slices here, four more than the RP2040's eight, which
//! moves the block-wide registers from `+0xa0` to `+0xf0` and widens the
//! per-channel masks in `EN` and the interrupt registers from 8 bits to 12
//! (RP2350 datasheet table 1054).
//!
//! Slices 8 to 11 have no pin on this package. They are reachable only from
//! GPIO32 and above, which the QFN-60 part the Pico 2 uses does not bond out,
//! so [`ChannelNumber`] can name them but [`From<RPGpio>`] never yields one.
...
register_structs! {
    PwmRegisters {
        ...
        (0x00F8 => inte: ReadWrite<u32, CH::Register>),
        // Interrupt force register
        (0x00FC => intf: ReadWrite<u32, CH::Register>),
        // Interrupt status after masking & forcing
        (0x0100 => ints: ReadOnly<u32, CH::Register>),
        (0x0104 => @END),
    }
}
...
/// Channel identifier
///
/// There are twelve PWM slices on the RP2350. Slices 8 to 11 have no pin on
/// the QFN-60 package, so they can be named and driven but their output
/// reaches nothing.
...
///
/// GPIO 0 to 15 map to slices 0 to 7 in order, and GPIO 16 to 29 repeat that
/// pattern, so the slice is bits 3:1 of the pin number on every pin this
/// package has. Slices 8 to 11 begin at GPIO32 and are unreachable here, which
/// is why this only ever returns `Ch0` to `Ch7`.
