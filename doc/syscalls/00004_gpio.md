...
## Command
  ...
  * ### Command number: `5`
    ...
    **Returns**: `Ok(())` if the pin identifier is valid, `INVAL` if it is
    invalid, and `NOSUPPORT` if the resistor configuration is not supported by
  ...
  * ### Command number: `7`
    ...
    **Returns**: `Ok(())` if the pin identifier is valid, `INVAL` if it is
    ...
    `NOSUPPORT` if an invalid interrupt mode is passed in the
...
Unused for the GPIO driver. Will always return `NOSUPPORT`.
