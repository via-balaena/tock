...
///
/// This board has no LED to blink. GPIO 25 carries the user LED on a
/// Raspberry Pi Pico 2, but on a Pico 2 W it is the CYW43439 radio's SPI chip
/// select, and the LED is on the radio itself. So the panic message goes out
/// over the console and the board halts, as the Raspberry Pi Pico W does for
/// the same reason.
#[cfg(not(test))]
#[panic_handler]
