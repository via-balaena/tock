...
use qemu_rv64_virt_chip::chip::{QemuRv64VirtChip, QemuRv64VirtDefaultPeripherals};
use rv64i::csr;
use rv64i::dma_fence::RiscvCoherentDmaFence;

pub mod io;

pub const NUM_PROCS: usize = 4;

pub type ChipHw = QemuRv64VirtChip<'static, QemuRv64VirtDefaultPeripherals<'static>>;
...
type RngDriver = components::rng::RngRandomComponentType<
    qemu_rv64_virt_chip::virtio::devices::virtio_rng::VirtIORng<
        'static,
        'static,
        RiscvCoherentDmaFence,
    >,
>;
pub type ScreenHw = qemu_rv64_virt_chip::virtio::devices::virtio_gpu::VirtIOGPU<
    'static,
    'static,
    RiscvCoherentDmaFence,
>;

type AlarmHw = qemu_rv64_virt_chip::chip::QemuRv64VirtClint<'static>;
...
pub struct QemuRv64VirtPlatform {
    ...
    alarm: &'static capsules_core::alarm::AlarmDriver<
        'static,
        VirtualMuxAlarm<'static, qemu_rv64_virt_chip::chip::QemuRv64VirtClint<'static>>,
    >,
    ...
    virtio_ethernet_tap: Option<
        &'static capsules_extra::ethernet_tap::EthernetTapDriver<
            'static,
            qemu_rv64_virt_chip::virtio::devices::virtio_net::VirtIONet<
                'static,
                RiscvCoherentDmaFence,
            >,
        >,
    ...
    pub virtio_input_keyboard: Option<
        &'static qemu_rv64_virt_chip::virtio::devices::virtio_input::VirtIOInput<
            'static,
            RiscvCoherentDmaFence,
        >,
    >,
...
impl SyscallDriverLookup for QemuRv64VirtPlatform {
...
impl
    KernelResources<
        qemu_rv64_virt_chip::chip::QemuRv64VirtChip<
            'static,
            QemuRv64VirtDefaultPeripherals<'static>,
        >,
    > for QemuRv64VirtPlatform
{
...
pub unsafe fn start() -> (
    ...
    QemuRv64VirtPlatform,
    &'static qemu_rv64_virt_chip::chip::QemuRv64VirtChip<
        'static,
        QemuRv64VirtDefaultPeripherals<'static>,
    >,
) {
    ...
    // protection.
    // let epmp = rv64i::pmp::kernel_protection_mml_epmp::KernelProtectionMMLEPMP::new(
    //     rv64i::pmp::kernel_protection_mml_epmp::FlashRegion(
    //         rv64i::pmp::NAPOTRegionSpec::from_start_end(
    //             core::ptr::addr_of!(_sflash),
    //             core::ptr::addr_of!(_eflash),
    //         )
    //         .unwrap(),
    //     ),
    //     rv64i::pmp::kernel_protection_mml_epmp::RAMRegion(
    //         rv64i::pmp::NAPOTRegionSpec::from_start_end(
    ...
    //         .unwrap(),
    //     ),
    //     rv64i::pmp::kernel_protection_mml_epmp::MMIORegion(
    //         rv64i::pmp::NAPOTRegionSpec::from_start_size(
    //             core::ptr::null::<u8>(), // start
    //             0x20000000,              // size
    //         )
    //         .unwrap(),
    //     ),
    //     rv64i::pmp::kernel_protection_mml_epmp::KernelTextRegion(
    //         rv64i::pmp::TORRegionSpec::from_start_end(
    ...
    // ---------- QEMU-SYSTEM-RISCV64 "virt" MACHINE PERIPHERALS ----------
    ...
    let mux_alarm = static_init!(
        MuxAlarm<'static, qemu_rv64_virt_chip::chip::QemuRv64VirtClint>,
    ...
    let virtual_alarm_user = static_init!(
        VirtualMuxAlarm<'static, qemu_rv64_virt_chip::chip::QemuRv64VirtClint>,
    ...
    for (i, virtio_device) in peripherals.virtio_mmio.iter().enumerate() {
        use qemu_rv64_virt_chip::virtio::devices::VirtIODeviceType;
    ...
    > = if let Some(gpu_idx) = virtio_gpu_idx {
        use qemu_rv64_virt_chip::virtio::devices::virtio_gpu::{
            MAX_REQ_SIZE, MAX_RESP_SIZE, PIXEL_STRIDE, VirtIOGPU,
        };
        use qemu_rv64_virt_chip::virtio::queues::Virtqueue;
        use qemu_rv64_virt_chip::virtio::queues::split_queue::{
            SplitVirtqueue, VirtqueueAvailableRing, VirtqueueDescriptors, VirtqueueUsedRing,
        };
        use qemu_rv64_virt_chip::virtio::transports::VirtIOTransport;
    ...
    let virtio_rng: Option<
        &'static qemu_rv64_virt_chip::virtio::devices::virtio_rng::VirtIORng<RiscvCoherentDmaFence>,
    > = if let Some(rng_idx) = virtio_rng_idx {
        use qemu_rv64_virt_chip::virtio::devices::virtio_rng::VirtIORng;
        use qemu_rv64_virt_chip::virtio::queues::Virtqueue;
        use qemu_rv64_virt_chip::virtio::queues::split_queue::{
            SplitVirtqueue, VirtqueueAvailableRing, VirtqueueDescriptors, VirtqueueUsedRing,
        };
    ...
    > = if let Some(net_idx) = virtio_net_idx {
        ...
        use qemu_rv64_virt_chip::virtio::devices::virtio_net::VirtIONet;
        use qemu_rv64_virt_chip::virtio::queues::Virtqueue;
        use qemu_rv64_virt_chip::virtio::queues::split_queue::{
            SplitVirtqueue, VirtqueueAvailableRing, VirtqueueDescriptors, VirtqueueUsedRing,
        };
    ...
    > = if let Some(input_idx) = virtio_input_idx {
        use qemu_rv64_virt_chip::virtio::devices::virtio_input::VirtIOInput;
        use qemu_rv64_virt_chip::virtio::queues::Virtqueue;
        use qemu_rv64_virt_chip::virtio::queues::split_queue::{
            SplitVirtqueue, VirtqueueAvailableRing, VirtqueueDescriptors, VirtqueueUsedRing,
        };
    ...
    let chip = static_init!(
        QemuRv64VirtChip<QemuRv64VirtDefaultPeripherals>,
        QemuRv64VirtChip::new(peripherals, hardware_timer, pmp),
    );
    PANIC_RESOURCES.get().map(|resources| {
    ...
    let platform = QemuRv64VirtPlatform {
        pconsole,
        console,
        alarm,
        lldb,
    ...
    debug!("QEMU RISC-V 64-bit \"virt\" machine, initialization complete.");
